  ...
  static std::expected<IntRatio, std::string> FromFloat(
    ...
    for (int dp = 0; dp < 32; ++dp) {
      ...
      if (new_acceptable && error >= 0) {
        ...
        // that is further from zero. Positive errors are preferred because
        // any negative error guarantees an off-by-one error due to integer
        // rounding. This means we bias very slightly positive.
